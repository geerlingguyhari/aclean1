...
CSV_SUFFIX = ".csv"
KEEP_LATEST = 10
LOGIN_TIMEOUT = 300       # 5 minutes timeout for login command
COLLECT_TIMEOUT = 300     # 5 minutes timeout for oc get ns command
SLEEP_BETWEEN_CLUSTERS = 300  # 5 minutes pause *between* clusters
MAX_RETRIES = 3
...
def main():
    ...
    for index, cluster in enumerate(clusters):
        print(f"\n[INFO] Processing cluster {index + 1}/{len(clusters)}: {cluster}")

        if login_to_cluster_with_retry(cluster, username, password):
            print(f"[INFO] Collecting namespace info for {cluster}...")
        ...
        if index < len(clusters) - 1:
            print(f"[INFO] Sleeping for {SLEEP_BETWEEN_CLUSTERS // 60} minutes before next cluster...")
