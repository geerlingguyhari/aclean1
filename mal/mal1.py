...
CSV_SUFFIX = ".csv"
KEEP_LATEST = 10  # Number of recent CSV files to keep
MAX_WORKERS = 5   # Parallelism level
    ...
    if output:
        try:
            ...
            for ns in namespaces:
                ...
                tia = labels.get("tia") or annotations.get("tia")
                maintainer = labels.get("Maintainer") or annotations.get("Maintainer")
...
def main():
    ...
    # CHOOSE ONE: (uncomment the one you prefer)
    filename = write_csv(merged_data, append=False)  # Timestamped CSVs (recommended with cleanup)
    # filename = write_csv(merged_data, append=True)   # Appending to single file (no cleanup needed)

    # Perform cleanup ONLY for timestamped CSVs
