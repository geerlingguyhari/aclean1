...
def save_existing_kubeconfig(cluster_name):
    os.makedirs(KUBECONFIG_DIR, exist_ok=True)
    src_kubeconfig = os.path.expanduser("~/.kube/config")
    if os.path.exists(src_kubeconfig):
        dest_kubeconfig = os.path.join(KUBECONFIG_DIR, f"kubeconfig_{cluster_name.replace('https://', '').replace(':', '_').replace('/', '_')}")
        shutil.copyfile(src_kubeconfig, dest_kubeconfig)
        print(f"[INFO] Using existing ~/.kube/config, saved as: {dest_kubeconfig}")
        return dest_kubeconfig
    else:
        print(f"[ERROR] ~/.kube/config not found for cluster {cluster_name}. Please login manually first.")
        return None
...
def main():
    ...
    for cluster in clusters:
        print(f"[INFO] Please ensure you have logged into: {cluster} manually (if needed).")
        kubeconfig = save_existing_kubeconfig(cluster)
        if kubeconfig:
            kubeconfig_files.append(kubeconfig)

    if not kubeconfig_files:
        print("[ERROR] No kubeconfigs available. Exiting.")
        return
