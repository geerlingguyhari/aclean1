...
def generate_all_kubeconfigs(clusters, username, password):
    ...
    for cluster in clusters:
        ...
        oc_login_to_generate_kubeconfig(cluster, username, password, kubeconfig_path)

def process_cluster(cluster):
    kubeconfig_path = os.path.join(KUBECONFIG_DIR, f"kubeconfig_{os.path.basename(cluster)}")
    return extract_namespace_info(cluster, kubeconfig_path)
...
def main():
    ...
    generate_all_kubeconfigs(clusters, username, password)

    print("[INFO] Starting parallel data extraction from clusters...")
    results = []
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_cluster = {
            executor.submit(process_cluster, cluster): cluster for cluster in clusters
        }
