...
def process_secret(secret, serviceid, password):
    ...
    for b64_val in list(data.values()) + list(string_data.values()):
        if isinstance(b64_val, dict):
            # Handle case where value might be a nested dictionary
            sid_found, matched = match_credentials(b64_val, serviceid, password)
            if matched == 'yes':
                break
            continue
                
        decoded = decode_b64(b64_val, nested=True) if isinstance(b64_val, str) else str(b64_val)
        if not decoded:
            continue
        ...
        try:
            ...
            # If not JSON, check if it's a raw credential string
            if ':' in decoded:
                parts = decoded.split(':', 1)
                if (len(parts) == 2 and 
                    parts[0].strip().lower() == serviceid.lower() and 
                    parts[1].strip() == password.strip()):
                    sid_found = parts[0].strip()
                    matched = 'yes'
                    break
...
def process_namespace(kubeconfig_path, cluster_url, ns, serviceid, password):
    ...
    for secret in secrets:
        ...
        if sid or match == 'yes':  # Include all secrets that were checked, not just matches
            rows.append([cluster_url, namespace, name, stype, sid, match])
            if match == 'yes':
                logging.info(f"✅ Match in secret '{name}' (type: {stype}, ns: {namespace}, cluster: {cluster_url})")
            else:
                logging.debug(f"Checked secret '{name}' (type: {stype}, ns: {namespace})")
    return rows
