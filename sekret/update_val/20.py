...
def check_credential_pair(key, value, serviceid_to_check, user_password, is_base64=True):
    ...
    if ':' in decoded_value:
        ...
        if len(parts) == 2:
            uid, pwd = parts
            if (uid.strip().lower() == serviceid_to_check.strip().lower() and 
                pwd.strip() == user_password.strip()):
                return uid, 'yes'
    
    # Check if the key looks like it might contain a username or password
    key_lower = key.lower()
...
def process_secret(secret, serviceid_to_check, user_password):
    ...
    if secret_type == 'Opaque':
        # Check all key-value pairs in data (base64 encoded)
        for key, val in data.items():
            sid, match = check_credential_pair(key, val, serviceid_to_check, user_password, is_base64=True)
            if match == 'yes':
                return namespace, name, secret_type, sid, match
            elif match == 'partial' and matched != 'yes':
                sid_found, matched = sid, match
        
        # Check all key-value pairs in stringData (plain text)
                ...
                try:
                    ...
                    if ':' in decoded:
                        ...
                        if len(parts) == 2:
                            ...
                            if (uid.strip().lower() == serviceid_to_check.strip().lower() and 
                                pwd.strip() == user_password.strip()):
                                return namespace, name, secret_type, uid, 'yes'
...
def process_namespace(kubeconfig, cluster_url, ns, serviceid_to_check, user_password):
    ...
    for secret in secrets:
        ...
        if sid or stype in ['Opaque', 'kubernetes.io/dockerconfigjson', 'kubernetes.io/dockercfg']:
            rows.append([cluster_url, namespace, name, stype, sid if sid else 'N/A', match])
            if match == 'yes':
                logging.info(f"✅ Match in secret '{name}' (ns: {namespace}, type: {stype})")
            elif match == 'partial':
                logging.info(f"⚠️ Partial match in secret '{name}' (ns: {namespace}, type: {stype})")
            else:
                logging.debug(f"ℹ️ Checked secret '{name}' (ns: {namespace}, type: {stype})")
