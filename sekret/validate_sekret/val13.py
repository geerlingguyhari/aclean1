...
def parse_possible_dockerconfig(decoded_bytes, cluster_url, namespace, secret_name):
    try:
        ...
        for _, details in auths.items():
            ...
            if username.lower() == normalized_provided_serviceid:
                return "Yes", "Yes" if aggressive_normalize(password.encode("utf-8")) == normalized_provided_password else "No"

            if auth_b64:
                u, p = decode_auth_field(auth_b64)
                if u and u.strip().lower() == normalized_provided_serviceid:
                    return "Yes", "Yes" if aggressive_normalize(p.encode("utf-8")) == normalized_provided_password else "No"
...
def process_secret_data(data, cluster_url, namespace, secret_name):
    ...
    for key, encoded_value in data.items():
        ...
        if key in [".dockerconfigjson", ".dockercfg"] or decoded_text.strip().startswith("{"):
            sid, pwd = parse_possible_dockerconfig(decoded_bytes, cluster_url, namespace, secret_name)
            if sid == "Yes":
                return sid, pwd
