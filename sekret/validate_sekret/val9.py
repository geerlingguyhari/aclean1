...
normalized_provided_serviceid = service_id.strip().lower()
...
def decode_and_check_credentials(details, cluster_url, namespace, secret_name):
    ...
    if DEBUG and (username_match or password_match):
        print(f"\n[DEBUG] Docker Secret | {cluster_url} | {namespace} | {secret_name}")
        print(f"Username (provided vs. secret): {normalized_provided_serviceid} vs {username}")
        print(f"Password (provided vs. secret hex): {normalized_provided_password.hex()} vs {aggressive_normalize(password.encode('utf-8')).hex()}")

    # Also check auth if available
...
def process_namespace(kubeconfig, cluster_url, namespace):
    ...
    for secret in secrets:
        ...
        if secret_type == "kubernetes.io/dockerconfigjson":
            dockerconfig_b64 = data.get(".dockerconfigjson", "")
            try:
                dockerconfig_json = json.loads(base64.b64decode(dockerconfig_b64).decode('utf-8'))
                auths = dockerconfig_json.get("auths", {})
            ...
            dockercfg_b64 = data.get(".dockercfg", "")
            try:
                dockercfg_json = json.loads(base64.b64decode(dockercfg_b64).decode('utf-8'))
                for reg, details in dockercfg_json.items():
                    username_match, pwd_match = decode_and_check_credentials(details, cluster_url, namespace, secret_name)
                    if username_match:
                        service_id_found = "Yes"
                        password_match = "Yes" if pwd_match else "No"
            ...
            for key, encoded_value in data.items():
                ...
                try:
                    ...
                    if normalized_provided_serviceid in decoded_text.lower():
                        ...
                        if DEBUG:
                            print(f"\n[DEBUG] Opaque Secret | {cluster_url} | {namespace} | {secret_name}")
                            print(f"Provided (hex): {normalized_provided_password.hex()}")
