...
def process_namespace(kubeconfig, cluster_url, namespace):
    ...
    for secret in secrets:
        ...
        if secret_type == "kubernetes.io/dockerconfigjson":
        ...
        else:  # Handle Opaque and other secret types
            for key, encoded_value in data.items():
                ...
                # 1️⃣ Direct service ID string in decoded data
                ...
                # 2️⃣ Check for base64-encoded 'auth' fields that decode to username:password
                if key.lower() == "auth":
                    try:
                ...
                # 3️⃣ Explicit 'username' / 'password' fields in Opaque secrets
