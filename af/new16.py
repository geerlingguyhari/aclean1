...
def send_individual_folder_email(folder_data, recipients, custom_body=None, reminder_text=""):
    ...
    - Bold image count
    - Link styling
    - Sorted attachments by date with size in MB
    """
    ...
    <div class="email-container">
        ...
        <div class="content">
            ...
            if old_images and include_attachments:
                ...
                old_images_csv = [
                    ["Image Path", "Created Date", "Size (MB)"]  # Updated header
                ]
            ...
            # Add main folder image details CSV
            if include_attachments:
                output_csv_data = []
                output_file = get_writable_path(f"{folder_data['folder']}_output_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
                try:
                    with open(output_file, 'r') as f:
                        reader = csv.DictReader(f)
                        # Filter for this folder's data and convert to list for sorting
                        folder_rows = []
                        for row in reader:
                            if row['Main Folder'].strip() == folder_data['folder']:
                                try:
                                    # Parse the created date for sorting
                                    ...
                                    folder_rows.append({
                                        'row': row,
                                        'created_date': datetime.min  # Default to oldest if date parsing fails
                                        })
 
                        # Sort by Created date (oldest first)
                        folder_rows_sorted = sorted(folder_rows, key=lambda x: x['created_date'])
                        
                        # Prepare CSV data with MB size
                        output_csv_data.append(["Repository", "Main Folder", "Image Path", "Created", "Last Used", "Size (MB)"])
                        for item in folder_rows_sorted:
                            ...
                            output_csv_data.append([
                                ...
                                row['Created'],
                                row['Last Used'],
                                row['Size (MB)']  # Already in MB from process_image function
                            ])
                except Exception as e:
                    logger.error(f"Error reading output file for attachments: {e}")
