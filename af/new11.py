...
def send_email_report(csv_file, folder_choice, report_scope):
    ...
    - Clean format for summary reports (All Folders)
    - Action-oriented format for filtered reports
    ...
    try:
        ...
        # Calculate total old images count (for filtered reports)
        total_old_images = sum(len(old_images_data.get(item['folder'], [])) for item in data) if report_scope != "All Folders" else 0

        # Only include cleanup message for filtered reports
        if report_scope != "All Folders":
            cleanup_html = f"""
<div class="cleanup-notice"> style="margin-top: 10px; margin-bottom: 10px; line-height: 1.6;">
...
"""
        ...
        # Prepare HTML content with appropriate styling
        html = f"""<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    <table>
        ...
        # Set appropriate subject based on report type
        if report_scope == "All Folders":
            msg['Subject'] = f"Artifactory Storage Report - {datetime.now().strftime('%b %d, %Y')}"
        else:
