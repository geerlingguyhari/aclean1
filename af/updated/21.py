...
<body>
    ...
    <table>
        ...
        to_email = EMAIL_TO.split(',')[0].strip() if EMAIL_TO else EMAIL_FROM
        msg['To'] = to_email
        
        # Add CC emails if multiple provided in EMAIL_TO
        cc_emails = [e.strip() for e in EMAIL_TO.split(',')[1:]] if EMAIL_TO else []
        if cc_emails:
...
def send_individual_folder_email(folder_data, recipients, custom_body=None, reminder_text=""):
    ...
    cc_emails = [e.strip() for e in DEFAULT_EMAIL.split(',')[1:]] if DEFAULT_EMAIL else []
...
<body>
    <div class="email-container">
        ...
        <div class="content">
            ...
            logger.info(f"Successfully sent individual report for {folder_data['folder']} to {msg['To']}" + (f", CC: {msg['Cc']}" if cc_emails else ""))
            return True
