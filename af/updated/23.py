...
<body>
    ...
    <table>
        ...
        # Parse TO and CC emails - keep original TO behavior
        email_to_list = [e.strip() for e in EMAIL_TO.split(',')] if EMAIL_TO else []
        to_email = email_to_list[0] if email_to_list else EMAIL_FROM
        msg['To'] = to_email
        
        # Add CC emails if multiple provided in EMAIL_TO
        cc_emails = email_to_list[1:] if len(email_to_list) > 1 else []
        if cc_emails:
...
def send_individual_folder_email(folder_data, recipients, custom_body=None, reminder_text=""):
    ...
    default_email_list = [e.strip() for e in DEFAULT_EMAIL.split(',')] if DEFAULT_EMAIL else []
    cc_emails = default_email_list[1:] if len(default_email_list) > 1 else []
    
    # Keep original TO recipient behavior
    to_email = recipients[0] if recipients else DEFAULT_EMAIL.split(',')[0].strip() if DEFAULT_EMAIL else EMAIL_FROM
        ...
        try:
            ...
            if os.path.exists(total_size_file) and os.path.getsize(total_size_file) > 0:
                ...
                if folder_data:
                    ...
                    to_email = recipients[0] if recipients else default_emails[0] if default_emails else EMAIL_FROM
                    cc_emails = default_emails[1:] if len(default_emails) > 1 else []
