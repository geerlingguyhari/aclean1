...
1. <a href="abc.va.com/gcops">PCP RES Virtual Assistant</a>
2. <a href="xyz.api.com">Artifactory-Cleanup API</a>
For more details refer <a href="old.images.com">clean_up_old_images_from_Artifactory</a>. To engage: <a href="123.support.com">ABC Support</a> | <a href="dfg.request.com">My request</a>"""
CLEANUP_DAYS = 180
...
def send_email_report(csv_file, folder_choice, report_scope):
    """
    Final refined email report with all requested updates:
    - New subject format
    ...
    try:
        ...
        # Calculate total old images count
        total_old_images = sum(len(old_images_data.get(item['folder'], [])) for item in data)
        cleanup_html = f"""
<div class="cleanup-notice">
    <strong>⚠️ Cleanup Recommendation:</strong><br><br>
    {CLEANUP_MESSAGE}<br><br>
    <strong>Found {total_old_images} images older than {CLEANUP_DAYS} days across all folders</strong>
</div>
"""
...
<body>
    <div class="email-container">
        ...
        # Create and send email with updated subject
        msg = MIMEMultipart()
        msg['From'] = EMAIL_FROM
        msg['To'] = EMAIL_TO
...
def send_individual_folder_email(folder_data, recipients, custom_body=None, reminder_text=""):
    ...
    while attempt < max_attempts:
        try:
            ...
            msg['Subject'] = f"[Actions Required]: Request for Artifactory Storage Cleanup for TIA: {folder_data['folder']}"
...
<div class="cleanup-notice">
    ...
    2. <a href="xyz.api.com">Artifactory-Cleanup API</a><br>
    For more details refer <a href="old.images.com">clean_up_old_images_from_Artifactory</a>. To engage: <a href="123.support.com">ABC Support</a> | <a href="dfg.request.com">My request</a><br><br>
...
def main():
    ...
    if folder_choice.lower() == "all":
        ...
        # Get reminder option for single folder processing
        ...
        try:
            ...
            if os.path.exists(total_size_file) and os.path.getsize(total_size_file) > 0:
                # Read the folder size from the CSV
                with open(total_size_file, 'r') as f:
                    reader = csv.DictReader(f)
                    for row in reader:
...
Further details are provided below. Thank you for your cooperation.
"""
                    # Send individual email with custom body and reminder text
                    if send_individual_folder_email(folder_data, recipients, custom_body, reminder_text):
                        print("Email sent successfully with reminder")
            else:
                print("Error: Total size file not created properly, skipping email")
        except Exception as e:
