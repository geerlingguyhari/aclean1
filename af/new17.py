                        ...
                        try:
                            ...
                            if created_date < cutoff_date:
                                old_images.append({
                                    'path': item_path,
                                    'created': created_str,
                                    'created_date': created_date,  # Add datetime object for sorting
...
def send_individual_folder_email(folder_data, recipients, custom_body=None, reminder_text=""):
    """Send email for individual folder report with all original functionality plus fixes for:
    - Date sorting in attachments (oldest to newest)
    - Size conversion from bytes to MB in old images CSV
    - File handling issues
    - Email size limits handling
    """
    ...
    # Separate primary recipients (from csv) and default email
    ...
    while attempt < max_attempts:
        try:
            ...
            # Cleanup recommendation with clickable links
            cleanup_html = f"""
    ...
    <div class="email-container">
        ...
        <div class="content">
            ...
            if old_images and include_attachments:
                # Sort old images by created date (oldest first)
                old_images_sorted = sorted(old_images, key=lambda x: x['created_date'])
                
                old_images_csv = [
                    ["Image Path", "Created Date", "Size (MB)"]  # Updated header with MB
                ]
                for img in old_images_sorted:
                    ...
                    old_images_csv.append([
                        img['path'],
                        img['created'],
                        f"{size_mb:.2f}"  # Converted size in MB with 2 decimal places
                    ])
            ...
            # Add main folder image details CSV (sorted by date)
            if include_attachments:
                try:
                    # Find the most recent output file in the correct directory
                    output_dir = '/tmp' if os.access('/tmp', os.W_OK) else os.path.expanduser('~')
                    output_files = [
                        f for f in os.listdir(output_dir) 
                        if f.startswith('artifactory_data_') and f.endswith('.csv')
                    ]
                    ...
                    if output_files:
                        ...
                        latest_file = max(
                            output_files,
                            key=lambda x: datetime.strptime(x.split('_')[2].split('.')[0], "%Y%m%d_%H%M%S")
                        )
                        ...
                        with open(output_file_path, 'r') as f:
                            ...
                            folder_rows = []
                            for row in reader:
                                if row['Main Folder'].strip() == folder_data['folder']:
                                    try:
                                        # Parse created date for sorting
                                        created_date = datetime.strptime(row['Created'].split('.')[0], "%Y-%m-%dT%H:%M:%S")
                                        folder_rows.append({
                                            'data': row,
                                            'created_date': created_date
                                        })
                                    except (ValueError, KeyError):
                                        # Fallback to minimum date if parsing fails
                                        folder_rows.append({
                                            'data': row,
                            ...
                            folder_rows_sorted = sorted(folder_rows, key=lambda x: x['created_date'])
                            
                            # Prepare CSV data
                            output_csv_data = [
                                ["Repository", "Main Folder", "Image Path", "Created", "Last Used", "Size (MB)"]
                            ]
                            for item in folder_rows_sorted:
                                row = item['data']
                                output_csv_data.append([
                                    ...
                                    row['Created'],
                                    row['Last Used'],
                                    row['Size (MB)']  # Already in MB from process_image
                                ])
                            ...
                            attachment = MIMEText(csv_buffer.getvalue(), 'plain')
                            attachment.add_header('Content-Disposition', 'attachment',
                                               filename=f"{folder_data['folder']}_image_details_{datetime.now().strftime('%Y%m%d')}.csv")
                            msg.attach(attachment)
                    else:
                        logger.warning(f"No output files found in {output_dir}")
                except Exception as e:
                    logger.error(f"Error creating image details attachment: {e}")
            ...
            logger.info(f"Successfully sent report for {folder_data['folder']} to {msg['To']}" + 
                       (f", CC: {msg['Cc']}" if cc_recipients else ""))
            return True

        except smtplib.SMTPDataError as e:
            if "exceeds size limit" in str(e) and include_attachments:
                logger.warning("Email too large, retrying without attachments...")
                include_attachments = False
                attempt += 1
                continue
...
def main():
    ...
    if folder_choice.lower() == "all":
        ...
        print("3. Reminder 2")
        print("4. Reminder 3")
        reminder_option = input("Enter your choice (1-3): ").strip() or "1"
        ...
        try:
            ...
            if os.path.exists(total_size_file) and os.path.getsize(total_size_file) > 0:
                ...
                if folder_data:
                    # Load email mappings and send email
                    email_mappings = load_email_mappings()
...
Further details are provided below. Thank you for your cooperation.
"""
                    if send_individual_folder_email(folder_data, recipients, custom_body, reminder_text):
