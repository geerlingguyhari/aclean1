...
def collect_artifactory_data(base_url, path, auth, main_folder, writer, version_path=None):
    ...
    if response.status_code != 200:
        print(f"Error: Received {response.status_code} for URL: {url}")
        return 0
...
def calculate_total_size(base_url, path, auth):
    ...
    if 'children' in content:
        for item in content['children']:
            if not item['folder']:
                item_details_response = requests.get(f"{base_url}{path}{item['uri']}", auth=auth, verify=False)
                if item_details_response.status_code != 200:
                    continue
                item_data = safe_json_decode(item_details_response)
                if item_data:
    ...
    if folder_choice.lower() == "all":
        ...
        with open(lock_file, "w") as lf:
            try:
                ...
                response = requests.get(repo_base_url, auth=(username, password), verify=False)
                if response.status_code != 200:
                    print(f"Error: Received {response.status_code} for URL: {repo_base_url}")
                    return
